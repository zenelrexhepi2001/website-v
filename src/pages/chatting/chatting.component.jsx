...
function Chat() {
  ...
  const Hiddenchat = (e) => {
    e.preventDefault();
    document.querySelector(".chat-card").style.opacity = "0";
    document.querySelector(".chat-card").style.transition = "0.3s ease-in";

    window.addEventListener("load", () => {
      document.querySelector(".chat-card").style.opacity = "1";
      document.querySelector(".chat-card").style.transition = "0.3s ease-in";
    });
        ...
        <header className="chat-card-header">
          <div className="chat-card-user">
            ...
            <div className="card-chat-user-info">
              <h3>
                Shefket Robelli <span>Support advisor</span>
              </h3>
              <p>Online Typing...</p>
            </div>
          </div>
  ...
  return (
    <>
      <div className={`${dataClientChat.right} chat-card-client`}>
        <h1 className={`${dataClientChat.color}`}>
          {dataClientChat.description}
        </h1>
        <p>20:20</p>
      </div>
    </>
